      ...
      <b-colxx xxs="6">
        <b-form>
          <b-form-group label="Name">
            <b-form-input type="text" v-model="sensorForm.name" />
          </b-form-group>
          <b-form-group label="Sensor type">
            <v-select
              v-model="sensorForm.sensor_type_id"
              label="name"
              index="id"
              :options="sensorTypes"
            />
        ...
        <div class="mt-5">
          ...
          <b-button size="lg" variant="primary" @click="createSensor"
            >Submit</b-button
          >
        </div>
      </b-colxx>
  ...
  methods: {
    loadSensorTypes() {
      sensorTypeService.read().then(response => {
        ...
        this.sensorForm.sensor_type_id = this.sensorTypes[0] ? this.sensorTypes[0].id : null
      });
    },
    createSensor() {
      let id = this.$route.params.id;
      let payload = { name: this.sensorForm.name }
      let config = {
        device_id: id,
        sensor_type_id: this.sensorForm.sensor_type_id
      };

      sensorService.create(config, payload).then(response => {
        this.errors = [];
        if (response.errors) {
